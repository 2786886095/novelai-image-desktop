    ...
    <AppPortal>
      <div className="modal-backdrop reference-preset-dialog-backdrop">
        <section className="modal reference-preset-quick-dialog">
          <header><h2>{text.quickSave}</h2><button aria-label={text.cancel} onClick={onClose}><Icon name="close" /></button></header>
...
export default function ReferencePresetManager({
  ...
  const createPanel = (
    <section className="reference-preset-create panel-card">
      <header className="reference-preset-section-heading"><div><h3>{text.createTitle}</h3><p>{text.createHint}</p></div><button className="reference-preset-close" type="button" onClick={() => setShowCreate(false)} aria-label={text.cancel}><Icon name="close" /></button></header>
      ...
      <div className="reference-preset-create-fields">
        ...
        {kind === "precise" && <div className="field reference-preset-field-wide"><span>{text.typeLabel}</span><SelectMenu value={preciseType} ariaLabel={text.typeLabel} options={[{ value: "character", label: text.character }, { value: "style", label: text.style }, { value: "character&style", label: text.both }]} onChange={(next) => setPreciseType(next as PreciseReferenceType)} /><small>{text.typeHelp}</small></div>}
  ...
  return <>{modal ? <AppPortal><div className="modal-backdrop reference-preset-manager-backdrop"><div className="reference-preset-manager-modal">{content}</div></div></AppPortal> : content}{showCreate && <AppPortal><div className="modal-backdrop reference-preset-create-backdrop"><div className="reference-preset-create-modal">{createPanel}</div></div></AppPortal>}{previewPreset && <AppPortal><div className="modal-backdrop reference-preset-preview-backdrop" onClick={() => setPreviewPreset(null)}><div className="reference-preset-preview" onClick={(event) => event.stopPropagation()}><button className="reference-preset-close" type="button" onClick={() => setPreviewPreset(null)} aria-label={text.cancel}><Icon name="close" /></button><PreviewImageViewer images={presets.map(item=>({src:item.fileUrl,alt:localizedPresetName(item)}))} index={Math.max(0,presets.findIndex(item=>item.id===previewPreset.id))} onIndex={index=>setPreviewPreset(presets[index])}/><strong>{localizedPresetName(previewPreset)}</strong></div></div></AppPortal>}{deleteGroupTarget && <AppPortal><div className="reference-ui-mask reference-preset-confirm-mask" onClick={() => setDeleteGroupTarget(null)}><section className="reference-ui-dialog reference-preset-confirm-dialog" role="alertdialog" aria-modal="true" aria-labelledby="reference-delete-group-title" onClick={(event) => event.stopPropagation()}><div className="reference-ui-dialog-head"><strong id="reference-delete-group-title" className="reference-ui-dialog-title">{navText.deleteGroupTitle}</strong></div><div className="reference-ui-dialog-body"><b>{catalogGroupName(deleteGroupTarget, language)}</b><p>{navText.deleteGroupHint}</p></div><div className="reference-ui-dialog-actions"><button type="button" className="reference-ui-dialog-button reference-ui-dialog-button-default" onClick={() => setDeleteGroupTarget(null)}>{text.cancel}</button><button type="button" className="reference-ui-dialog-button reference-ui-dialog-button-primary" disabled={busy} onClick={() => void deleteGroup()}>{navText.deleteGroup}</button></div></section></div></AppPortal>}</>;
}
