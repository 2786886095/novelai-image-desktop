...
export default function ReferenceCatalogPanel({ library, onDownloaded }: { library: ReferencePresetLibrary; onDownloaded?: () => void }) {
  ...
  const changePage = async (targetPage: number) => {
    ...
    await Promise.allSettled(nextAssets.map((asset) => preloadCatalogImage(
      catalogThumbnailUrls(asset)[0],
      catalogThumbnailUrls(asset)[1],
    )));
  ...
  return <>
    ...
    {previewAsset && <AppPortal><div className="modal-backdrop reference-catalog-preview-backdrop" onClick={() => setPreviewAsset(null)}><div ref={previewRef} className="reference-catalog-preview" onClick={(event) => event.stopPropagation()}><button type="button" className="reference-preset-close" aria-label={text.close} onClick={() => setPreviewAsset(null)}><Icon name="close" /></button><PreviewImageViewer images={assets.map(item=>({src:item.id,alt:catalogName(item,language)}))} index={Math.max(0,assets.findIndex(item=>item.id===previewAsset.id))} onIndex={index=>setPreviewAsset(assets[index])} renderImage={<img src={catalogThumbnailUrls(previewAsset)[0]} onError={(event) => { const fallback = catalogThumbnailUrls(previewAsset)[1]; if (fallback && event.currentTarget.src !== fallback) event.currentTarget.src = fallback; }} alt={catalogName(previewAsset, language)} />}/><div className="reference-catalog-preview-copy"><strong>{catalogName(previewAsset, language)}</strong><span>{localizedGame(previewAsset.game)} · {localizedCategory(previewAsset.category)} · {previewAsset.width}×{previewAsset.height}</span></div></div></div></AppPortal>}
