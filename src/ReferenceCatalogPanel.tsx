...
export default function ReferenceCatalogPanel({ library, onDownloaded }: { library: ReferencePresetLibrary; onDownloaded?: () => void }) {
  ...
  return <>
    <section ref={panelRef} className="reference-catalog-panel panel-card weui-panel">
      ...
      {error && <div className="reference-catalog-error weui-cell weui-cell_warn"><span className="weui-cell__hd"><Icon name="warning" /></span><span className="weui-cell__bd"><strong>{text.unavailable}</strong><small>{error}</small></span><span className="weui-cell__ft"><button type="button" className="weui-btn weui-btn_mini weui-btn_default" onClick={() => void load(true)}>{text.retry}</button></span></div>}
      ...
      {catalog && <>
        <section className="reference-catalog-filter-shell" aria-label={text.search}>
          <div className="weui-search-bar reference-catalog-search" role="search"><div className="weui-search-bar__form"><div className="weui-search-bar__box"><Icon name="search" className="reference-catalog-search-icon" /><input className="weui-search-bar__input" type="search" value={query} placeholder={text.search} aria-label={text.search} onChange={(event) => setQuery(event.target.value)} />{query && <button className="reference-catalog-search-clear" type="button" aria-label={text.close} onClick={() => setQuery("")}><Icon name="close" /></button>}</div></div></div>
          <div className="reference-catalog-toolbar">
            <SelectMenu defaultOpen={captureSelectOpen} className="reference-catalog-select" value={game} ariaLabel={text.game} label={text.game} options={[{ value: "__all__", label: text.all }, ...gameOptions.map((item) => ({ value: item.id, label: localizedGame(item.id) }))]} onChange={(next) => { setGame(next); setCategory("__all__"); }} />
            {(game === "__all__" || availableCategories.length > 1) && <SelectMenu className="reference-catalog-select" value={category} ariaLabel={text.category} label={text.category} options={[{ value: "__all__", label: text.all }, ...availableCategories.map((item) => ({ value: item, label: localizedCategory(item) }))]} onChange={setCategory} />}
        ...
        {game === "__all__" ? <div className="reference-catalog-series-placeholder weui-cell"><span className="weui-cell__hd"><Icon name="info" /></span><span className="weui-cell__bd">{text.selectSeries}</span></div> : <section ref={seriesRef} className="reference-catalog-series-card">
