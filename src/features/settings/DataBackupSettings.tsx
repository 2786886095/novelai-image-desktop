...
const TEXT = {
  ...
  "zh-TW": {
    portabilityTitle: "跨端資料匯入與匯出", portabilityDesc: "建立桌面、Android 與 iOS 共用的 .naisbackup；預設全選，可逐項排除。", sensitive: "API Token 與第三方金鑰會依選擇直接寫入備份，請只交給可信裝置。", selectAll: "全選", clear: "清空", export: "匯出所選資料", exporting: "正在整理備份…", chooseImport: "選擇備份檔", importing: "正在安全合併…", importSelected: "匯入所選資料", archive: "待匯入封存", firstConfirmTitle: "確認合併所選資料", firstConfirm: "相同圖片依內容略過；同名不同圖自動加 (1)；記錄與時間群組只合併。匯入前會先建立完整安全備份。", configConfirmTitle: "再次確認覆蓋設定", configConfirm: "設定/API 類會覆蓋現有對應設定（裝置路徑除外）；其它資料仍只合併。", cancel: "取消", continue: "繼續", overwrite: "確認覆蓋並匯入", noCategory: "請至少選擇一類資料。", autoTitle: "自動備份與還原", autoDesc: "預設開啟並採用輕量模式，不會在啟動後壓縮整套圖庫。", autoEnabled: "啟用自動備份", autoEnabledDesc: "啟動後背景檢查；匯入前一定另建安全備份。", includeImages: "自動備份包含圖片與參考圖", includeImagesDesc: "開啟後耗時與空間占用會增加；手動匯出不受影響。", interval: "備份間隔（小時）", retention: "保留自動備份數量", directory: "備份目錄", chooseDirectory: "選擇目錄", openDirectory: "開啟目錄", backupNow: "立即完整備份", refresh: "更新狀態", latest: "最近備份", none: "尚無自動備份", count: "{count} 個自動備份 · {size}", config: "應用設定", api: "API 與敏感資料", artists: "畫家串與獨立收藏", textHistory: "反推與轉換歷史", references: "參考預設與參考圖", images: "本機圖片與生成記錄", prompts: "提示詞、風格與預設圖", workspace: "工具專案與其它本機資料",
  },
  "en-US": {
