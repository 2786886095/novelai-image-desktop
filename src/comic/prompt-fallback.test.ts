...
import { buildComicLocalPrompt, isComicPromptRefusal } from "./prompt-fallback";

describe("comic local prompt fallback", () => {
  it("maps common Chinese scene cues into stable English NovelAI tags", () => {
    const prompt = buildComicLocalPrompt({
      mode: "tags",
  ...
  it("recognizes common Chinese and English refusal responses", () => {
    expect(isComicPromptRefusal("Sorry, I can't help with that request.")).toBe(true);
    expect(isComicPromptRefusal("抱歉，我无法协助生成该内容。")).toBe(true);
    expect(isComicPromptRefusal("1girl, night, rain, cinematic lighting")).toBe(false);
  });
});
