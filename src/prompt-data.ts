...
const CAT_LABEL_LOCALE: Record<string, Record<number, string>> = {
  "zh-CN": CAT_LABEL,
  "zh-TW": { 0: "通用", 1: "畫師", 3: "作品", 4: "角色", 5: "元資訊" },
  "en-US": { 0: "General", 1: "Artist", 3: "Copyright", 4: "Character", 5: "Meta" },
  "ja-JP": { 0: "一般", 1: "絵師", 3: "作品", 4: "キャラ", 5: "メタ" },
  "ko-KR": { 0: "일반", 1: "작가", 3: "작품", 4: "캐릭터", 5: "메타" },
};

const COMMON_TAG_GLOSS: Record<string, Record<string, string>> = {
  "zh-CN": {
    "1girl": "一个女孩",
    "1boy": "一个男孩",
    solo: "单人",
...
const TAG_TOKEN_GLOSS: Record<string, Record<string, string>> = {
  "ja-JP": {
    girl: "女の子",
    girls: "女の子",
    boy: "男の子",
...
const CAPSULE_CATEGORY_LOCALE: Record<string, Record<string, string>> = {
  "en-US": {
    人物: "People",
    服饰: "Clothing",
    表情: "Expression",
...
const CAPSULE_SUBGROUP_LOCALE: Record<string, Record<string, string>> = {
  "en-US": {
    对象: "Subject",
    身份: "Role",
    年龄: "Age",
...
function normalizeLanguage(language: unknown): string {
  return language === "zh-TW" || language === "en-US" || language === "ja-JP" || language === "ko-KR" ? language : "zh-CN";
}

function readableTag(tag: string): string {
  return tag.trim().replace(/[_-]+/g, " ").replace(/\s+/g, " ").trim();
}

function splitReadableTag(tag: string): string[] {
  const expanded = readableTag(tag)
    .replace(/(\d+\+?)(girls?|boys?|others?)/gi, "$1 $2")
    .replace(/([a-z])(\d+)/gi, "$1 $2")
    .toLowerCase();
  return expanded.split(/[\s/()]+/).filter(Boolean);
}

function localizedReadableTag(tag: string, code: string): string {
  const readable = readableTag(tag);
  const normalized = readable.toLowerCase();
  const exact = COMMON_TAG_GLOSS[code]?.[normalized];
  if (exact) return exact;
  if (code === "en-US") return readable;
  const tokenGloss = TAG_TOKEN_GLOSS[code];
  if (!tokenGloss) return readable;
  const words = splitReadableTag(tag);
  if (!words.length) return readable;
  const localized = words.map((word) => tokenGloss[word] ?? word);
  return localized.join(code === "ja-JP" ? "・" : " ");
}

function fallbackCapsuleName(code: string, kind: "category" | "subgroup") {
...
function toTraditional(input: string): string {
  const map: Record<string, string> = {
    简: "簡", 体: "體", 发: "髮", 头: "頭", 马: "馬", 龙: "龍", 国: "國", 风: "風", 画: "畫", 图: "圖", 场: "場",
    质: "質", 词: "詞", 饰: "飾", 装: "裝", 连: "連", 袜: "襪", 肤: "膚", 动: "動", 势: "勢", 构: "構",
    视: "視", 觉: "覺", 对: "對", 龄: "齡", 负: "負", 单: "單", 个: "個", 蓝: "藍", 绿: "綠", 红: "紅",
    观: "觀", 众: "眾", 师: "師", 标: "標", 签: "籤", 库: "庫", 云: "雲", 气: "氣", 间: "間", 内: "內",
    园: "園", 学: "學",
  };
  return Array.from(input).map((char) => map[char] ?? char).join("");
}

export function localizedCategoryLabel(category: number, language?: unknown): string {
  const code = normalizeLanguage(language);
  return CAT_LABEL_LOCALE[code]?.[category] ?? CAT_LABEL[category] ?? (code === "zh-TW" ? "標籤" : code === "en-US" ? "Tag" : "Tag");
}

export function localizedCapsuleCategoryName(name: string, language?: unknown): string {
  const code = normalizeLanguage(language);
  if (code === "zh-CN") return name;
  if (code === "zh-TW") return toTraditional(name);
  return CAPSULE_CATEGORY_LOCALE[code]?.[name] ?? fallbackCapsuleName(code, "category");
}

export function localizedCapsuleSubgroupName(name: string, language?: unknown): string {
  const code = normalizeLanguage(language);
  if (code === "zh-CN") return name;
  if (code === "zh-TW") return toTraditional(name);
  return CAPSULE_SUBGROUP_LOCALE[code]?.[name] ?? fallbackCapsuleName(code, "subgroup");
}

export function localizedTagLabel(tag: string, sourceLabel?: string, language?: unknown): string {
  const code = normalizeLanguage(language);
  const normalized = readableTag(tag).toLowerCase();
  if (code === "zh-CN") return sourceLabel?.trim() || TAG_ZH[normalized] || COMMON_TAG_GLOSS["zh-CN"][normalized] || readableTag(tag);
  if (code === "zh-TW") return toTraditional(sourceLabel?.trim() || TAG_ZH[normalized] || COMMON_TAG_GLOSS["zh-CN"][normalized] || readableTag(tag));
  return localizedReadableTag(tag, code);
}
...
export function tagDescription(s: TagSuggestion, language?: unknown): string {
  return localizedTagLabel(s.tag, s.description, language) || `${localizedCategoryLabel(s.category, language)} category`;
}
