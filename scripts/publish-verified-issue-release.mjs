...
const tools=fs.readdirSync(buildTools).filter(name=>/^\d+(\.\d+)+$/.test(name)).sort((a,b)=>b.localeCompare(a,undefined,{numeric:true})).map(name=>path.join(buildTools,name)).find(dir=>fs.existsSync(path.join(dir,'apksigner')));
...
const certificate=file=>call(path.join(tools,'apksigner'),['verify','--print-certs',file]).match(/Signer #1 certificate SHA-256 digest:\s*(\S+)/)?.[1];
...
const evidence={version,sha,mobileRun,mobileSourceSha:mobile.headSha,signingHash,assets:files.map(file=>({name:path.basename(file),size:fs.statSync(file).size,sha256:crypto.createHash('sha256').update(fs.readFileSync(file)).digest('hex')}))};
