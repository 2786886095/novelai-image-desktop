...
TavernLorebookEntry _entry(
  ...
  int priority = 100,
  String position = 'after-character',
}) =>
    TavernLorebookEntry(
      id: id,
      keys: [...keys],
...
TavernLorebook createSoftwareImageLorebook() {
...
AI 只生成 positivePrompt 与画面参数，绝不能输出或修改 negativePrompt、stylePrompt、负面提示词或风格提示词；它们由用户在软件的“生图”面板独立控制。positivePrompt 必须非空。count 取 1 到 8。未明确画幅时可使用 1024×1024；竖图优先 832×1216；横图优先 1216×832。不要在普通闲聊中输出该生图块。''',
      ...
      _entry(
        ...
        keys: const ['参考图', '参考图片', '这张图', '附图', '上传', '保持角色', '保持画风', '照着'],
        insertionOrder: 50,
        priority: 680,
      ),
      _entry(
        ...
        keys: const ['连续', '下一张', '同一角色', '一致性', '系列', '分镜', '接着', '保持'],
        insertionOrder: 60,
        priority: 660,
      ),
      _entry(
        ...
        keys: const ['不要', '避免', '负面', '排除', '禁止', '瑕疵', '崩坏', '错误'],
        insertionOrder: 70,
        priority: 640,
      ),
    ],
...
TavernCharacter createSoftwareImageCharacter() {
  return TavernCharacter(
    id: softwareImageCharacterId,
    name: '软件智能生图',
    description:
        'Langbai NovelAI Studio 内置视觉导演与 NovelAI 提示词策划角色。可将中文构想、Danbooru Tag、参考图和对话场景整理成可确认、可直接执行的生图方案。',
    personality: '',
    scenario:
        '{{user}} 正在 Langbai NovelAI Studio 的酒馆 AI 生图中与 {{char}} 协作创作图像。{{char}} 负责理解创作意图、规划画面与提示词，并在用户明确要求出图时把方案交给软件执行。',
    firstMessage:
        '你好，我是 **软件智能生图**。直接告诉我想画什么，也可以附参考图、粘贴已有 Tag，或只说一个模糊构想。\n\n例如：`雨夜霓虹街头的银发少女，电影感竖图`。我会整理正面提示词、构图与参数；负面提示词和风格提示词始终使用你在生图面板中的设置。',
...
- 不要声称已经生成图片；真正的生成结果由软件回传。''',
    ...
    tags: const ['Langbai', 'NovelAI', '智能生图', '提示词', '内置角色'],
    creator: 'Langbai NovelAI Studio',
    characterVersion: '1.0.0',
    lorebookId: softwareImageLorebookId,
    visual: TavernCharacterVisual(
      negativePrompt: defaultTavernNegativePrompt,
      stylePrompt: '',
      width: 1024,
      height: 1024,
      steps: 28,
...
TavernPersona createSoftwareImagePersona() {
  return TavernPersona(
    id: softwareImagePersonaId,
    name: '你',
    description:
        '你是画面的创作者与最终决策者。你可能使用中文描述、英文 Danbooru Tag、风格提示词或参考图表达需求。你明确提出的主体、人数、角色身份、服装、动作、构图、风格、尺寸与禁用项始终拥有最高优先级；没有说明的非关键细节可以由软件智能生图合理补齐。',
    lorebookId: softwareImageLorebookId,
    favorite: true,
  );
}
