import '../agent/lyra_preset_data.dart';
...
const lyraImageTaskSourceSha256 = lyraPresetSourceSha256;

String buildDshImageAiSystemAddon(
  DshImageAiTask task, {
  String mode = 'focused',
  bool includeSharedPreset = true,
  TavernSamplerPreset? sharedPreset,
  bool useDefaultSharedPreset = true,
}) {
  final effectivePreset = sharedPreset ??
      (useDefaultSharedPreset
          ? TavernSamplerPreset(
              name: lyraPresetName,
  ...
  return [
    if (includeSharedPreset && effectivePreset != null) ...[
...
String injectDshImageAiSystemPrompt({
  ...
  final effectiveSystemPrompt = sharedPreset?.systemPrompt ??
      (useDefaultSharedPreset ? lyraPresetSystemPrompt : '');
  final sharedPresetAlreadyPresent = effectiveSystemPrompt.isNotEmpty &&
      systemPrompt.contains(effectiveSystemPrompt);
  return [
    buildDshImageAiSystemAddon(
      task,
      mode: mode,
      includeSharedPreset: !sharedPresetAlreadyPresent,
      sharedPreset: sharedPreset,
      useDefaultSharedPreset: useDefaultSharedPreset,
    ),
    systemPrompt.trim(),
